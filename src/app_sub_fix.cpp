...
void Shutdown()
{
  system("sync;sync; echo odroid | sudo -S shutdown -h now");
}
...
void sensor_diagnosis_switch_on()
{
	system("rosrun sensor_diagnosis Imu_hz &");
	system("rosrun sensor_diagnosis LaserScan_hz &");
	system("rosrun sensor_diagnosis WheelFb_hz &");
}

void sensor_diagnosis_switch_off()
{
	system("rosnode kill Imu_hz LaserScan_hz WheelFb_hz");
}
...
void chatterCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  else if(tmp.compare("5")==0)
  {
	...
	{
				sensor_diagnosis_switch_on();
	}
	else if(tmp.compare("7")==0)
	{
				sensor_diagnosis_switch_off();
	}
  else
  {
  ROS_INFO("ERROR");
...
int main(int argc, char **argv)
{
  ...
  chatter_pub= n.advertise<actionlib_msgs::GoalID>("/move_base/cancel", 1000);
	ros::spin();
